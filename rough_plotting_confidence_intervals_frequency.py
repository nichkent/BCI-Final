...
# -*- coding: utf-8 -*-
"""
Created on Mon May  6 18:36:22 2024
...
def plot_power_spectrum(spectra_by_class, fft_frequencies, subject, channels_to_plot, class_to_plot=None):
    ...
    if class_to_plot != None:
        ...
        for channel in channels_to_plot:
            ...
            plt.plot(fft_frequencies, spectra_by_class[class_to_plot-1][channel,:]) # Class labels are 1-indexed, Python is 0-indexed
...
def plot_multiple_power_spectra(spectra_by_class1, spectra_by_class2, fft_frequencies1, fft_frequencies2, subject, channels_to_plot, class_to_plot, activity1='Active', activity2='Rest'):
...
def plot_power_spectra_confidence_intervals(eeg_epochs_fft_active, eeg_epochs_fft_rest, spectra_by_class_active, spectra_by_class_rest, fft_frequencies_active, fft_frequencies_rest, subject, channels_to_plot, class_to_plot, activity1='Active', activity2='Rest'):
    ...
    for plot_index, channel in enumerate(channels_to_plot): # plot_index to access a subplot
        ...
        channel_plot[plot_index].fill_between(fft_frequencies_active, spectra_by_class_active[class_to_plot-1][channel,:] - 2 * active_se_mean[channel], spectra_by_class_active[class_to_plot-1][channel,:] + 2 * active_se_mean[channel], alpha=0.2, label='Target +/- 95% CI')
        
        # Add confidence intervals for the rest frequency spectrum
        channel_plot[plot_index].fill_between(fft_frequencies_rest, spectra_by_class_rest[class_to_plot-1][channel,:] - 2 * rest_se_mean[channel], spectra_by_class_rest[class_to_plot-1][channel,:] + 2 * rest_se_mean[channel], alpha=0.2, label='Rest +/- 95% CI')
